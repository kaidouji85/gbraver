...
test.describe('testClientテスト', function() {
    ...
    test.it('リクエストパラメータに指定したコードがViewに埋め込まれる', function(){
        var testCodeName = '/javascripts/battleSceneTest/attackByPlayer.js';
        ...
        driver.findElement(webdriver.By.name('testCode')).getAttribute('src').then(function(src){
            assert.equal(src,'http://localhost:3000/javascripts/battleSceneTest/attackByPlayer.js','正しいテストコードがScriptタグに格納されている');
        });
    });
});